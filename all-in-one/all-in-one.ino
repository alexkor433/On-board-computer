  ...
  if (ms - tmr >= 1000) {
    ...
    if (!Hold) {
      ...
      if (prevT1 != t1) {               // печатаем на дисплее только если изменилась температура
        ...
        lcd.print(t1);
        switch (t1) {
          case 10: case 100: lcd.print(FPSTR(degC)); break; // печатаем символ градуса, если температура увеличилась на разряд
          case 9: case 99: lcd.print(FPSTR(degCgap)); break;// если температура уменьшилась на разряд, печатаем символ градуса, а предыдущий очищаем
        }
        prevT1 = t1;
      }
#ifdef TWO_CYLINDERS
      if (prevT2 != t2) {
        ...
        lcd.print(t2);
        switch (t2) {
          case 10: case 100: lcd.print(FPSTR(degC)); break;
          case 9: case 99: lcd.print(FPSTR(degCgap)); break;
        }
        prevT2 = t2;
      }
#endif
