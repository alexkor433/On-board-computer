...
GyverMAX6675<thermoSCK, thermoSO, thermoCS> thermo; // указываем пины в порядке SCK SO CS
#ifdef TwoCylinders
GyverMAX6675<thermoSCK2, thermoSO2, thermoCS2> thermo2;
#endif
LiquidCrystal_I2C lcd(lcdAddr, 16, LINES);          // адрес, сегменты и строки дисплея
GyverTM1637 disp(CLK, DIO);
Tachometer tacho;
EncButton<EB_TICK, A, B, keypin> enc(INPUT);        // энкодер с кнопкой <A, B, KEY> (A, B, KEY - номера пинов)
...
#if defined ecoRPM
const char name9[] PROGMEM = "ecoRPM note";
#endif
...
void setup() {
  ...
  minVMH = minV - 0.8;                // из мин. напряжения вычитаем 0.8 вольт, чтобы моточасы записывались только при выключении
    ...
    if (enc.held()) { // смена режимов показа на дисплее
      ...
      switch (Hold) {
        ...
        case 1:         // при переходе в режим настройки
          analogWrite (ledpin, pgm_read_byte(&(CRTgammaPGM[vals[CRT_VALS]])));
#if defined ecoRPM
          analogWrite (ecoledpin, pgm_read_byte(&(CRTgammaPGM[vals[CRT_VALS]])));
#endif
...
#ifdef RPMwarning
        if ((uint16_t)millis() - myTimer4 > 1400 && R >= 5800) {
...
void isButtonSingle() {             // действия после ОДИНОЧНОГО нажатия кнопки
  digitalWrite(ledpin, LOW);
  disp.displayByte(_U, _2, _9, _6); // выводим версию программы на дисплей
  lcd.clear();
    ...
    switch (controlState) {
      ...
      case 1:                                 // управляем ПАРАМЕТРАМИ
        ...
        switch (arrowPos) {   // ограничиваем только изменённые настройки
          ...
          case 5: vals[5] = constrain(bool(vals[5]), 0, 1);           // тест пищалки
...
# ifdef ecoRPM
            analogWrite (ecoledpin, pgm_read_byte(&(CRTgammaPGM[vals[6]])));
# endif
            break;
          case 7: if (vals[7]) { // если равно единице обнуляем счётчик моточасов
              vals[7] = 0;
              e_hours = 0;
              EEPROM.put(0, e_hours);
            }
            break;
# ifdef ecoRPM
          case 8: vals[8] = constrain(bool(vals[8]), 0, 1); break; // включение предупреждения eco-оборотов
# endif
        }

#elif defined noPiezo// для системы БЕЗ пьезоэлемента:
        switch (arrowPos) {
          case 0: vals[0] = constrain(vals[0], 0, 7); break;
...
# ifdef ecoRPM
            analogWrite (ecoledpin, pgm_read_byte(&(CRTgammaPGM[vals[4]])));
# endif
            break;
          case 5: if (vals[5]) {
              vals[5] = 0;
